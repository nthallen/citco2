...

{
	...
	printf("\n%s ready\n", devDescriptors[descriptorIndex].devString);
  {
    ...
    int n_ports;
    int port;
    err = ulDIOGetInfo(daqDeviceHandle,DIO_INFO_NUM_PORTS, 0, &InfoVal);
    if (err != ERR_NO_ERROR) {
      printf("Error getting NUM_PORTS\n");
      n_ports = 0;
    } else {
      n_ports = (int)InfoVal;
      printf("Board has %d ports\n", n_ports);
    }
    for (port = 0; port < n_ports; ++port) {
      uint8_t DIval;

      err = ulDIOGetInfo(daqDeviceHandle, DIO_INFO_PORT_TYPE, port, &InfoVal);
      portType = (DigitalPortType)InfoVal;
      ConvertPortTypeToString(portType, portTypeStr);

      err = ulDIOGetInfo(daqDeviceHandle, DIO_INFO_PORT_IO_TYPE, port, &InfoVal);
      portIoType = (DigitalPortIoType)InfoVal;
