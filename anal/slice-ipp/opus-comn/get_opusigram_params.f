...
c    slicnt    I*4    Slice counter, points into nptvec,bpdata,timsli,mnymxy
c
c  Output:
...
c
     ...
     & paramtyp    ! Type of OPUS parameters, used to check for existence
      ...
      real*4
     & mnymxy(msl,mch,2)! Subroutine output argument (see above)
     ...
     & dblval,     ! Temporary variable used in reading MNY, MXY
...
c
     ...
     &       ' data'
          elseif(ipdstat(channel).eq.0) then
            errnum=4
            write(*,'(5a)')'Error: ',path(1:lnbc(path)),
     &       ' has no ',chan(channel)(1:lnbc(chan(channel))),
...
c
     ...
     &         verbose,slicnt+dstatcnt,errnum,tpx)
            ...
            call get_opus_r8(luns,ipdstat(channel),'MNY',iendian,path,
     &       verbose,1,errnum,dblval)
            mnymxy(slicnt,channel,1)=real(dblval)
            call get_opus_r8(luns,ipdstat(channel),'MXY',iendian,path,
     &       verbose,1,errnum,dblval)
            mnymxy(slicnt,channel,2)=real(dblval)
...
c FIXME: sperate channel from detector and from AC/DC, then use these:
...
c      parameter (dtc_si     = 2)
     ...
     &       'RT-Si Diode DC').eq.1) then
            ...
            elseif(index(cval(1:strlen),'Bolometer').eq.1) then
              ...
              errnum=4
              write(*,'(4a)')'Error: ',path(1:lnbc(path)),
     &         ' has unexpected DTC of ',cval(1:strlen)
            endif
            if((errnum.eq.0).and.(verbose.ge.5)) then
              write(*,*) 'DTC_cooked=',i4head(i_dtccode)
            endif
          endif
...
c
        if((errnum.eq.0).and.(iporgpar.ne.0)) then
          call get_opus_string(luns,iporgpar,'SFM',typ_string,0,iendian,
     &     path,verbose,1,errnum,strlen,cval)
          if(errnum.eq.0) then
            i4head(i_sfmcode)=0
            if(index(cval(1:strlen),'Solar').eq.1)
...
c
     ...
     &     path,verbose,1,errnum,strlen,cval)
          if(errnum.eq.0) then
            i4head(i_inscode)=0
            if(index(cval(1:strlen),'IFS120').eq.1)
     &       i4head(i_inscode)=ins_120
            if(index(cval(1:strlen),'IFS120M').eq.1)
     &       i4head(i_inscode)=ins_120m
            if(index(cval(1:strlen),'IFS 125').eq.1)
     &       i4head(i_inscode)=ins_125
            if(index(cval(1:strlen),'IFS125 HR').eq.1)
     &       i4head(i_inscode)=ins_125
            if(index(cval(1:strlen),'IFS125HR').eq.1)
     &       i4head(i_inscode)=ins_125
            if(index(cval(1:strlen),'IFS66/S').eq.1)
     &       i4head(i_inscode)=ins_66s
            if(index(cval(1:strlen),
     &       'Undefined (No flange board found)').eq.1)
     &       i4head(i_inscode)=ins_125
            if(i4head(i_inscode).eq.0) then
              errnum=4
              write(*,'(4a)')'Error: ',path(1:lnbc(path)),
     &         ' has unexpected INS of ',cval(1:strlen)
            elseif(verbose.ge.5) then
              write(*,*) 'INS_cooked=',i4head(i_inscode)
            endif
          endif
     ...
     &       1,errnum,r8head(i_foc))
          elseif(errnum.ne.0) then  ! If previous error, don't rely on INS
            continue
          elseif((i4head(i_inscode).eq.ins_120).or.
     &           (i4head(i_inscode).eq.ins_125)) then
            r8head(i_foc)=418.d0
            if(verbose.ge.3) then
              write(*,'(3a)') path(1:lnbc(path)),' is missing FOC, ',
     &         'using 418 mm based on INS of IFS120HR or IFS125HR'
            endif
          elseif(i4head(i_inscode).eq.ins_120m) then
            r8head(i_foc)=215.d0
            if(verbose.ge.3) then
              write(*,'(3a)') path(1:lnbc(path)),' is missing FOC, ',
     &         'using 215 mm based on INS of IFS120M'
            endif
          elseif(i4head(i_inscode).eq.ins_66s) then
            r8head(i_foc)=153.5d0
            if(verbose.ge.3) then
              write(*,'(3a)') path(1:lnbc(path)),' is missing FOC, ',
     &         'using 153.5 mm based on INS of IFS66/S'
            endif
          else
            errnum=4
            write(*,'(2a)')'Error: could not determine FOC value for ',
     &       path(1:lnbc(path))
          endif
...
c
c Fix bad PKL in IFS125 with "VSN: 2.485 Apr  4 2017" and "VSD: M16SD   / 2.480"
c
            indexa=(tpx/2)-i4head(i_prl)
            if((slicnt.gt.1).and.(abs(indexa-i4head(i_pkl)).gt.100))then
              if(verbose.ge.3) then
                write(*,'(a,i11,a,i7)')'Replacing bad PKL of',
     &           i4head(i_pkl),' with',indexa
              endif
              i4head(i_pkl)=indexa
            endif
c
...
c
c Fix bad P2L in IFS125 with "VSN: 2.485 Apr  4 2017" and "VSD: M16SD   / 2.480"
c
            indexa=(tpx/2)-i4head(i_p2k)
            if((slicnt.gt.1).and.(abs(indexa-i4head(i_p2l)).gt.100))then
              if(verbose.ge.3) then
                write(*,'(a,i11,a,i7)')'Replacing bad P2L of',
     &           i4head(i_p2l),' with',indexa
              endif
              i4head(i_p2l)=indexa
            endif
c
...
c DW: Added kludge to account for M16 controller (20161012).
