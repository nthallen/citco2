     ...
     & i4head,r8head,infovec,tlalevel,errnum)
c
c  Input:
...
c
     ...
     &     tpx,time,i4head,r8head,infovec,tlalevel,errnum)
        endif    ! outfmt.eq.X
      endif
      return
      end
...
c
     ...
     &         delimit(1:1),stringy(fnbc(stringy):)
        ...
        elseif(minmax.eq.0) then
          do indexa=minind,maxind
            if(errnum.eq.0) then
              write(stringx,'(f13.6)')minfreq+(dble(indexa-1)*pointspac)
              write(stringy,'(f13.6)') buffer(indexa)
              write(luna,'(3a)',iostat=ierr) stringx(fnbc(stringx):),
     &         delimit(1:1),stringy(fnbc(stringy):)
              if(ierr.ne.0) then
                errnum=-10
...
c
        ...
        if(datype.eq.1) then
          ...
          if(i4head(i_gbw).gt.i4head(i_gfw)) then   ! Reverse run
            i4head(i_izpd)=tpx-i4head(i_izpd)
          endif 
        endif
c
...
c
        res=1.8d0*hfl/dble(tpx-i4head(i_izpd))
        phr=1.8d0*hfl/dble(i4head(i_nphr))
c
...
c
     ...
     &   pointr,header)
        ...
        if(i4head(i_inscode).eq.ins_120) then
          call pk_st_opus('INS','IFS120HR',typ_string,iend,
     &     mhl,errnum,pointr,header)
        elseif(i4head(i_inscode).eq.ins_120m) then
          call pk_st_opus('INS','IFS120M',typ_string,iend,
     &     mhl,errnum,pointr,header)
        elseif(i4head(i_inscode).eq.ins_125) then
          call pk_st_opus('INS','IFS125HR',typ_string,iend,
     &     mhl,errnum,pointr,header)
        elseif(i4head(i_inscode).eq.ins_66s) then
          call pk_st_opus('INS','IFS66/S',typ_string,iend,
     &     mhl,errnum,pointr,header)
        endif
...
c
     ...
     &   pointr,header)
        ...
        if(tlalevel.ge.tla_ext_min) then
          call pk_r8_opus('OPL',dble(tpx-i4head(i_izpd))/(2.d0*hfl),
     &     iend,mhl,errnum,pointr,header)
          call pk_r8_opus('OPS',dble(i4head(i_izpd))/(2.d0*hfl),
     &     iend,mhl,errnum,pointr,header)
        endif
        call pk_i4_opus('NSS',
...
c
     ...
     &     pointr,header)
          ...
          if(tlalevel.ge.tla_ext_min) then
            call pk_i4_opus('PCL',i4head(i_pcl),iend,mhl,errnum,
     &       pointr,header)
            call pk_r8_opus('PCT',r8head(i_pct),iend,mhl,errnum,
     &       pointr,header)
          endif
     ...
     &     pointr,header)
          ...
          if(tlalevel.ge.tla_ext_min) then
c In the OPUS format, the point indices are 0-based, hence "i4head(i_izpd)-1" 
            call pk_i4_opus('ZPL',i4head(i_izpd)-1,iend,mhl,errnum,
     &       pointr,header)
            call pk_r8_opus('ZPA',r8head(i_zpa),iend,mhl,errnum,
     &       pointr,header)
...
c      parameter (dtc_si     = 2)
        ...
        if((i4head(i_dtccode).eq.1).and.(channel.eq.1)) then
          call pk_st_opus('DTC','InGaAs-AC',typ_senum,iend,
     &     mhl,errnum,pointr,header)
        elseif((i4head(i_dtccode).eq.1).and.(channel.eq.2)) then
          call pk_st_opus('DTC','Si-AC',typ_senum,iend,
     &     mhl,errnum,pointr,header)
        elseif((i4head(i_dtccode).eq.2).and.(channel.eq.1)) then
          call pk_st_opus('DTC','InGaAs-DC',typ_senum,iend,
     &     mhl,errnum,pointr,header)
        elseif((i4head(i_dtccode).eq.2).and.(channel.eq.2)) then
          call pk_st_opus('DTC','Si-DC',typ_senum,iend,
     &     mhl,errnum,pointr,header)
        elseif(i4head(i_dtccode).eq.dtc_insb) then
          call pk_st_opus('DTC','InSb-AC',typ_senum,iend,
     &     mhl,errnum,pointr,header)
        elseif(i4head(i_dtccode).eq.dtc_bolom) then
          call pk_st_opus('DTC','Bolometer',typ_senum,iend,
     &     mhl,errnum,pointr,header)
c FIXME        elseif(i4head(i_dtccode).eq.4) then
c FIXME          call pk_st_opus('DTC','InSb-DC',typ_senum,iend,
c FIXME           mhl,errnum,pointr,header)
        endif
